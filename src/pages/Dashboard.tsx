...
const Dashboard: React.FC = () => {
  const statCards = [
    ...
    { icon: <ExclamationCircleIcon className="h-10 w-10" />, label: "ການລາຍງານ", value: 10 },
    ...
    { icon: <StarIcon className="h-10 w-10" />, label: "ການໃຫ້ຄະແນນ", value: 10 },
  ];
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-lao">
      ...
      {/* Welcome Banner */}
      <div className="bg-primary text-white rounded-3xl flex flex-col sm:flex-row items-center justify-between px-8 py-6 mb-10 mt-6 shadow-lg">
        <div>
      ...
      <div className="px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {statCards.map((card, index) => (
          <div
            key={index}
            className="bg-white text-primary rounded-lg flex flex-col items-center justify-center py-6 gap-3 shadow-md hover:shadow-lg hover:scale-[1.02] transition"
          >
            {card.icon}
